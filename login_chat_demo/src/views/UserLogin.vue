    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          //alert('submit!');
          this.isLoading=true;
          this.postRequest("/backendServer/userLogin",this.loginForm).then(resp=>{
              this.stopLoading();
              if(resp){
                ...
                window.sessionStorage.setItem('userToken',respTokenHead);
                this.$store.commit('setUserTokenInfo',respTokenHead);//记录token
                this.$store.commit('setAccountId',resp.obj.accountId);
                window.sessionStorage.setItem('userID',resp.obj.accountId);
                window.sessionStorage.setItem('username',resp.obj.username);
                window.sessionStorage.setItem('loginRole','roleUser');
